...
export default function TouristSpotsScreen({ headerHeight }) {
  const router = useRouter();
  const { touristSpots, loading, error, getAllTouristSpots } = useTouristSpotManager();
  ...
  const renderTouristSpotCard = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        activeOpacity={0.9}
        onPress={() => router.push(`/tourist/tourist_spots/tourist_spot_view?id=${item.id}`)}
      >
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {touristSpots.length === 0
                ? "No tourist spots found. Please check back later."
