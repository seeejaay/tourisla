...
export const GuideFeedbackModal = ({
  open,
  onClose,
  guideId,
  ...
  useEffect(() => {
    if (open && guideId) {
      ...
      setAnswers({});
      fetchMyGuideFeedbacks().then((groups) => {
      ...
      <View style={styles.overlay}>
        <View style={styles.container}>
          {feedbackGiven ? (
            ...
            <ScrollView>
              <Text style={styles.title}>Leave Feedback for {guideName}</Text>
              {questions.map((q) => (
              ...
              <TouchableOpacity
                ...
                disabled={
                  loading || questions.some((q) => !answers[q.id])
                }
              >
                {loading ? (
