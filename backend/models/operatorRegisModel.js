...
const createOperatorRegis = async (operatorRegisData) => {
  ...
  const result = await db.query(
    "INSERT INTO touroperator_applicants (operator_name, representative_name, email, mobile_number, office_address, application_status, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING *",
    [operator_name, representative_name, email, mobile_number, office_address, application_status]
  );
...
const editOperatorRegis = async (operatorId, operatorRegisData) => {
  ...
  const result = await db.query(
    ...
    [operator_name, representative_name, email, mobile_number, office_address, application_status, operatorId]
  );
  return result.rows[0];
};
...
const getOperatorRegisById = async (operatorId) => {
  const result = await db.query("SELECT * FROM touroperator_applicants WHERE id = $1", [
    operatorId,
  ]);
  return result.rows[0];
};
