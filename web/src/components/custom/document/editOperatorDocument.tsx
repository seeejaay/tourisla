...
type EditOperatorDocumentProps = {
  docuId: string;
  initialData: {
...
const EditOperatorDocument: React.FC<EditOperatorDocumentProps> = ({
  docuId,
  initialData,
  onSuccess,
  onCancel,
}) => {
  const { editOperatorDocument, loading, error } = useDocumentManager();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Only validate file if a new one is selected
    if (file) {
    ...
    try {
      const result = await editOperatorDocument(docuId, formData);
...
export default EditOperatorDocument;
