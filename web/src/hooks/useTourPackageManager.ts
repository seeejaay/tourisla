...
export const useTourPackageManager = () => {
  ...
  const fetchAll = useCallback(async (): Promise<TourPackage[]> => {
    ...
    try {
      ...
      console.log("API Tour packages fetched successfully");
      return packages;
    } catch (err) {
  ...
  const fetchOne = useCallback(
    async (id: number | string): Promise<TourPackage | null> => {
      ...
      try {
        ...
        console.log("API Tour package fetched successfully");
        return pkg;
      } catch (err) {
  ...
  const create = useCallback(
    async (data: Partial<TourPackage>): Promise<TourPackage | null> => {
      setLoading(true);
      setError("");
      console.log("Front end Creating tour package with data");
      try {
